        ...
        try:
            ...
            if not igrill:
                ...
                if status.retry_count >= config.max_retries_before_warning:
                    print(f"[ALERT] Reached max retries ({config.max_retries_before_warning}). Attempting to reset BLE adapter...")
                    reset_ble_adapter()
            ...
            try:
                ...
                while client.is_connected:
                    ...
                    await client.start_notify(config.temperature_uuid, lambda _, data: handle_notification(data, status, config))
